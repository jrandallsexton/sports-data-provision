...
foreach ($entry in $storageAccounts) {
    ...
    Write-Host "Checking storage account: $accountName" -ForegroundColor Yellow

    $existingAccount = az storage account show --name $accountName --resource-group $resourceGroup --query "name" --output tsv 2>$null

    if (-not $existingAccount) {
        Write-Host "Creating storage account: $accountName" -ForegroundColor Green
        az storage account create --name $accountName --resource-group $resourceGroup --sku Standard_LRS --kind StorageV2 --location eastus --output none
    }
    else {
    ...
    # Get the storage key
    $storageKey = az storage account keys list --account-name $accountName --resource-group $resourceGroup --query "[0].value" --output tsv

    # Ensure the container exists
    az storage container create --name $containerName --account-name $accountName --account-key $storageKey --output none
