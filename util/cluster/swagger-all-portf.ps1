...
Write-Host "  Provider: http://localhost:5001/swagger" -ForegroundColor Yellow
Write-Host "  Producer: http://localhost:5002/swagger" -ForegroundColor Yellow
Write-Host "  API:      http://localhost:5003/swagger" -ForegroundColor Yellow
Write-Host ""
...
try {
    ...
    $providerJob = Start-Job -ScriptBlock {
        param($pod)
        kubectl port-forward -n default pod/$pod 5001:8080
    } -ArgumentList $providerPod

    $producerJob = Start-Job -ScriptBlock {
        param($pod)
        kubectl port-forward -n default pod/$pod 5002:8080
    } -ArgumentList $producerPod

    $apiJob = Start-Job -ScriptBlock {
        param($pod)
        kubectl port-forward -n default pod/$pod 5003:8080
    } -ArgumentList $apiPod

    # Wait a moment for port forwards to establish
    Start-Sleep -Seconds 2

    # Open all three Swagger UIs in browser
    Write-Host "Opening Swagger UIs in browser..." -ForegroundColor Cyan
    ...
    Write-Host ""
    Write-Host "All Swagger UIs running. Press Ctrl+C to stop." -ForegroundColor Green
    Write-Host ""
