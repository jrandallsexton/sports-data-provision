#!/usr/bin/env pwsh
# Port forward to Hangfire dashboard on API pod

Write-Host "Finding API pod..." -ForegroundColor Cyan

# Get the API pod name
$podName = kubectl get pods -n default -o json | 
    ConvertFrom-Json | 
    Select-Object -ExpandProperty items | 
    Where-Object { $_.metadata.name -like "api-all-*" } | 
    Select-Object -First 1 -ExpandProperty metadata | 
    Select-Object -ExpandProperty name

if (-not $podName) {
    Write-Host "ERROR: No API pod found in default namespace" -ForegroundColor Red
    exit 1
}
...
Write-Host "Starting port forward to 8083..." -ForegroundColor Cyan
Write-Host ""
Write-Host "Hangfire dashboard will be available at: http://localhost:8083/dashboard" -ForegroundColor Yellow
Write-Host ""
...
try {
    ...
    kubectl port-forward -n default pod/$podName 8083:8080
}
finally {
    Write-Host ""
